...
def _validate_validator_cli_structure_independently(candidate_source: str) -> None:
    ...
    expected_contract_seals = {
        ...
        "CONTRACT_TEST_FUNCTION_INVENTORY_SHA256": (
            "c885c0b5845adafe82f90b05eaadf2af1ac88db9cc71be01660553da0063d09a"
        ),
        "FAILED_BOOTSTRAP_TEST_SHA256": (
...
'''
    ...
    with tempfile.TemporaryDirectory(prefix="mochirii-acme-transport-") as directory:
        ...
        unsafe_header_helper_fixture = transport_root / "acme-http-header-unsafe-owner"
        unsafe_header_helper_bytes = header_helper_fixture.read_bytes()
        unsafe_header_helper_fixture.write_bytes(unsafe_header_helper_bytes)
        unsafe_header_helper_fixture.chmod(0o755)
        if os.geteuid() == 0:
            os.chown(unsafe_header_helper_fixture, 65534, 65534)
        unsafe_header_helper_stat = unsafe_header_helper_fixture.stat()
        if unsafe_header_helper_stat.st_uid == 0 and unsafe_header_helper_stat.st_gid == 0:
            raise RuntimeError("Linux ACME unsafe-helper fixture unexpectedly has trusted ownership.")
        unsafe_header_helper_sha = hashlib.sha256(unsafe_header_helper_bytes).hexdigest()
        ...
        unsafe_helper_client_source = client_wrapper
        unsafe_helper_substitutions = (
            (
            ...
            (
                "/usr/local/libexec/mochirii-forums/acme-http-header",
                str(unsafe_header_helper_fixture),
            ),
            (
                VALIDATOR.ACME_HEADER_HELPER_SHA256,
                unsafe_header_helper_sha,
            ),
        )
        for original, replacement in unsafe_helper_substitutions:
            if unsafe_helper_client_source.count(original) != 1:
                raise RuntimeError("ACME unsafe-helper client fixture substitution seam differs.")
            unsafe_helper_client_source = unsafe_helper_client_source.replace(original, replacement, 1)
        unsafe_helper_client_fixture = transport_root / "mochirii-acme-client-unsafe-helper"
        unsafe_helper_client_fixture.write_text(
            unsafe_helper_client_source,
            encoding="utf-8",
            newline="\n",
        )
        unsafe_helper_client_fixture.chmod(0o755)
        unsafe_helper_client_result = subprocess.run(
            [str(unsafe_helper_client_fixture), "/usr/bin/env", "--"],
            env=hostile_parent_environment,
            stdin=subprocess.DEVNULL,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        ...
        if (
            unsafe_helper_client_result.returncode == 0
            or unsafe_helper_client_result.stdout
            or unsafe_helper_client_result.stderr != b"FORUMS_ACME_CLIENT_PREFLIGHT_FAILED\n"
            or unsafe_header_helper_fixture.read_bytes() != unsafe_header_helper_bytes
        ):
            raise RuntimeError("Actual ACME client wrapper modified or accepted an unsafe helper owner.")
        ...
        if (
            ...
            or any(
                name in observed_environment
                for name in hostile_parent_environment
                if name not in {"HOME", "PATH"}
            )
        ):
